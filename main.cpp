...
//        pixel_color += tracer->WhittedRayTrace(ray, scene, 0); //трассируем луч и получаем цвет
		int depth = 0;
        pixel_color = tracer->TraceRay(ray, scene, dot_light, depth); //трассируем луч и получаем цвет
      }
...
void test_scene1()
{
 ...
 auto plane1 = std::make_shared<Plane>(float3(+0.0f, -1.0f, +0.0f), float3(0.0f, 1.0f, 0.0f), new IdealMirror(float3(0.3f, 0.3f, 0.3f)));
  ...
  auto sph  = std::make_shared<Sphere> (float3(5.0f, +5.0f, +2.0f), 2,   new DiffusalMaterial(float3(0.0f, 1.0f, 1.0f)));
